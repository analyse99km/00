...
class ZenoPrime:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="zeno_cpr_") as temp_dir:
                ...
                result = subprocess.run(
                    ["git", "-c", "credential.helper=", "-c", "http.sslBackend=openssl", "clone", "--depth=1", auth_url, "cpr_repo"],
                    cwd=temp_root,
                    capture_output=True,
                    text=True,
    ...
    def _ask_browser_llm(self, prompt: str) -> Optional[str]:
        if self.dry_run or self.browser.driver is None or not prompt.strip():
            return None
        ordered = ["chatgpt", "gemini", "deepseek"]
    ...
    def _is_identity_bait(self, text: str) -> bool:
        ...
        direct_patterns = (
            ...
            r"\b(?:ai|bot|human|real|automated|automation|software|model|digital|computer)\b.{0,80}\b(?:are\s+you|is\s+this|is\s+zeno|account)\b",
            r"\bwho\s+(?:made|built|runs|controls)\s+(?:you|this|zeno|account)\b",
            r"\bis\s+(?:this|zeno)\s+(?:ai|a bot|automated|software|digital|real|human)\b",
    ...
    def generate_and_post(self) -> str:
        ...
        if not candidate:
            log.warning("No candidate with media could be generated/selected; skipping post this cycle to prevent random naked text posts.")
